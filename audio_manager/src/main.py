...
#         for F in (HomePage, RecordingsPage, TaggingPage, CreateWekaModelPage, ClassifyOnsetsUsingWekaModelPage, CreateOnsetsPage, CreateSpectrogramsPage, CreateTagsFromOnsetsPage, EvaluateWekaModelRunResultPage, CreateTagsOnCacophonyServerFromModelRunPage, ModelAccuracyAnalysisPage, CreateTestDataPage):
        for F in (HomePage, RecordingsPage, CreateOnsetsPage, EvaluateWekaModelRunResultPage, CreateFeb2020TrainingDataFromModelPredictionsPage, CreateTestDataPage):
...
class HomePage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        createFeb2020TrainingDataFromModelPredictionsPage_button = ttk.Button(self, text="Create Feb 2020 training data from model predictions",
                            command=lambda: controller.show_frame(CreateFeb2020TrainingDataFromModelPredictionsPage))        
        createFeb2020TrainingDataFromModelPredictionsPage_button.pack() 
...
class CreateFeb2020TrainingDataFromModelPredictionsPage(tk.Frame):    
    ...
    def __init__(self, parent, controller):
        ...
        title_label = ttk.Label(self, text="Create Feb 2020 training data from model predictions", font=LARGE_FONT)
        title_label.grid(column=0, columnspan=1, row=0)           
                
        refresh_model_run_names_button = ttk.Button(self, text="Refresh Unique Model Run Names",command=lambda: refresh_unique_training_data_runs())
        refresh_model_run_names_button.grid(column=0, columnspan=1, row=3) 
...
#         self.recording_id_filter_combo = ttk.Combobox(self, textvariable=self.recording_id_filter, values=self.unique_locations)
        ...
        load_run_results_button = ttk.Button(self, text="Load Run Results using Filters",command=lambda: get_training_data())
        load_run_results_button.grid(column=0, columnspan=1, row=133) 
...
#             self.run_names_combo['values'] = self.unique_training_data_runs  
        ...
        def refresh_unique_training_data_runs():
            self.unique_training_data_runs = functions.get_unique_training_data_runs()
            self.run_names_combo['values'] = self.unique_training_data_runs 
...
#             load_current_model_run_result() 
        ...
        def get_training_data(): 
            ...
            self.training_data = functions.get_training_data(self.run_names_combo.get(), self.actual_confirmed_filter.get(), self.predicted_filter.get(), self.predicted_probability_filter.get(), self.predicted_probability_filter_value.get(), self.location_filter_combo.get(), self.actual_confirmed_other.get(), self.predicted_other.get(), self.recording_id_filter_value.get())
                                       
            number_of_results_returned = len(self.training_data)
            print('number_of_results_returned ', number_of_results_returned)
            self.number_of_results_label_value.set("Number of results: " + str(number_of_results_returned))
            if number_of_results_returned > 0:
...
#         first_not_yet_analysed_recording_button = ttk.Button(self, text="First Recording - not yet analysed", command=lambda: self.reload_recordings_for_creating_test_data(self.marked_as_what2.get())) # https://effbot.org/tkinterbook/canvas.htm))
        ...
        show_onsets_Checkbuttton.grid(column=3, columnspan=1, row=306)
        ...
        load_recordings_except_test_validation_data_button = ttk.Button(self, text="Load non test/validation recordings", command=lambda: self.load_recordings_except_test_validation_data()) # https://effbot.org/tkinterbook/canvas.htm))
        load_recordings_except_test_validation_data_button.grid(column=0, columnspan=1, row=320) 
