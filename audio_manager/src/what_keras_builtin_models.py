...
#                                                               weights='imagenet',
#                                                               )
    inceptionResNetV2Model = tf.keras.applications.InceptionResNetV2(
        include_top=False, 
        weights='imagenet', 
        input_shape=IMG_SHAPE) 
    
    inceptionResNetV2Model.trainable=False
...
def create_model_Keras_builtin_NASNetLarge(binary, number_of_distinct_labels):
    ...
    IMG_SIZE=32
    IMG_SHAPE = (IMG_SIZE, IMG_SIZE, 3)
    NASNetLarge=tf.keras.applications.NASNetLarge(input_shape=IMG_SHAPE,
                                               include_top=False,
                                               weights='imagenet',
                                               )
...
def plot_confusion_matrix_3(binary, predictions_decoded, val_labels_decoded, integer_to_sound_mapping):
    ...
    print("len(unique_val_labels)", len(unique_val_labels))
    print("unique_val_labels", unique_val_labels)
  
    unique_predictions_labels = np.unique(predictions_decoded_np)
    print("unique_predictions_labels", unique_predictions_labels)
    

    labels = []
#     for value in unique_val_labels:
# #     for value in unique_predictions_labels:
...
def main():       
    ...
    run_sub_log_dir_multi_class = "keras_builtin_ResNet152_imagenet_4 " + "_multi_class"
    run_sub_log_dir_binary = "keras_builtin_ResNet152_imagenet_4" + "_binary"
    
    model_run_name = "2020_09_15_keras_builtin_ResNet152_v1"    
