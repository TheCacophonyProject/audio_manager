    ...
    for model_run_result in model_run_results:
        try:
            ...
            sql = ''' INSERT INTO tags(modelRunName, recording_id, startTime, duration, what, confidence, device_super_name, device_name, version, automatic, confirmed_by_human, created_locally, createdAt, tagger_username)
                          VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
            cur1.execute(sql, (modelRunName, recording_id, startTime, duration, predictedByModel, probability, device_super_name, device_name, parameters.model_version, automatic, confirmed_by_human, created_locally, createdAtDate, parameters.cacophony_user_name))
...
def retrieve_recordings_except_test_validation_data(model_must_predict_what):   
    ...
    firstDateOfTestValidationDataToExclude = parameters.recordings_for_creating_test_data_start_date 
    lastDateOfTestValidationDataToExclude = parameters.recordings_for_creating_test_data_end_date 
...
def get_recording_ids(firstDate, lastDate):
    cur = get_database_connection().cursor()
...
#     
#     try:
#         sql = ''' INSERT INTO model_run_result(model_run_name, recording_id,startTime, duration, predictedByModel, probability, device_name,device_super_name, recordingDateTime, recordingDateTimeNZ)
#                   VALUES(?,?,?,?,?,?,?,?,?,?) '''
#         cur = get_database_connection().cursor()
...
def save_prediction(model_run_name, recording_id, start_time_seconds, duration_seconds, predicted_by_model, probability):
    cur = get_database_connection().cursor()
    cur.execute("SELECT device_name, device_super_name, recordingDateTime, recordingDateTimeNZ FROM recordings WHERE recording_id = ? ", (recording_id,))
    result = cur.fetchall()
    ...
    try:
#         sql = ''' INSERT INTO model_run_results(version, recording_id, start_time_seconds, duration_seconds, predicted_by_model, probability, device_name, device_super_name, recordingDateTime, recordingDateTimeNZ)
#                   VALUES(?,?,?,?,?,?,?,?,?,?) '''
        sql = ''' INSERT INTO model_run_result(model_run_name, recording_id, startTime, duration, predicted_by_model, probability, device_name, device_super_name, recordingDateTime, recordingDateTimeNZ)
                  VALUES(?,?,?,?,?,?,?,?,?,?) '''
        cur = get_database_connection().cursor()
...
def update_model_run_result_table_with_latest_model_predictions_from_csv():
   
#     myFile = pd.read_csv('/home/tim/Work/Cacophony/Audio_Analysis/training_data_10717_recordings.csv', sep=',')
#     myFile = pd.read_csv('/home/tim/Work/Cacophony/Audio_Analysis/training_data_feb_2020_v2_cleaned_of_v1.csv', sep=',')
    myFile = pd.read_csv('/media/tim/HDD1/Temp/temp.csv')
    number_or_rows = len(myFile.index)
    count = 0
    for index, row in myFile.iterrows():
        count+=1
        print("Processing ", count, " of ", number_or_rows)
        model_run_name = row['model_run_name']
        recording_id = row['recording_id']
        ...
        try:
            sql = ''' INSERT INTO model_run_result(model_run_name, recording_id, startTime, duration, device_super_name, device_name, recordingDateTime,recordingDateTimeNZ, predicted_by_model, probability)
                  VALUES(?,?,?,?,?,?,?,?,?,?) '''
            cur = get_database_connection().cursor()
            cur.execute(sql, (model_run_name, recording_id, startTime, duration, device_super_name, device_name, recordingDateTime,recordingDateTimeNZ, predicted_by_model, probability))                            
            get_database_connection().commit()
        except Exception as e:
            print(e, '\n')
